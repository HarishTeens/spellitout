...
function getSpeaker(words) {
  ...
  const speakers = ["Harish", "Surya", "Sanjay"];
  console.log(words);

  return speakers[words[0]?.speaker];
}
    ...
    deepgram.addListener("transcriptReceived", async (packet) => {
      ...
      switch (type) {
        case "Results":
          ...
          socket.emit("transcript", `Speaker ${getSpeaker(data.channel.alternatives[0].words)} : ${await translateText(transcript)}`);
          break;
        case "Metadata":
