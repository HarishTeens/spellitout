...
class WhisperTimestampedASR(ASRBase):
    ...
    def transcribe(self, audio, init_prompt=""):
        result = whisper_timestamped.transcribe_timestamped(
            self.model, audio, language=self.original_language, initial_prompt=init_prompt, verbose=None, condition_on_previous_text=True)
        return result
...
class FasterWhisperASR(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        if model_dir is not None:
            print(
                f"Loading whisper model from model_dir {model_dir}. modelsize and cache_dir parameters are not used.", file=sys.stderr)
        ...
        model = WhisperModel(model_size_or_path, device="cuda",
                             compute_type="float32", download_root=cache_dir)
...
#        model = WhisperModel(modelsize, device="cpu", compute_type="int8") #, download_root="faster-disk-cache-dir/")
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        segments, info = self.model.transcribe(audio, language=self.original_language, initial_prompt=init_prompt,
                                               beam_size=5, word_timestamps=True, condition_on_previous_text=True, **self.transcribe_kargs)
...
class OnlineASRProcessor:
    ...
    def process_iter(self):
        ...
        print(
            f"transcribing {len(self.audio_buffer)/self.SAMPLING_RATE:2.2f} seconds from {self.buffer_time_offset:2.2f}", file=sys.stderr)
        ...
        print(">>>>COMPLETE NOW:", self.to_flush(
            o), file=sys.stderr, flush=True)
        print("INCOMPLETE:", self.to_flush(
            self.transcript_buffer.complete()), file=sys.stderr, flush=True)
...
# self.silence_iters = 0
        ...
        print(
            f"len of buffer now: {len(self.audio_buffer)/self.SAMPLING_RATE:2.2f}", file=sys.stderr)
...
def create_tokenizer(lan):
    ...
    assert lan in WHISPER_LANG_CODES, "language must be Whisper's supported lang code: " + \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='large-v2', choices="tiny.en,tiny,base.en,base,small.en,small,medium.en,medium,large-v1,large-v2,large".split(
    ...
    parser.add_argument('--task', type=str, default='transcribe',
                        choices=["transcribe", "translate"], help="Transcribe or translate.")
    ...
    parser.add_argument('--backend', type=str, default="faster-whisper", choices=[
                        "faster-whisper", "whisper_timestamped"], help='Load only this backend for Whisper processing.')
    parser.add_argument('--offline', action="store_true",
                        default=False, help='Offline mode.')
    parser.add_argument('--comp_unaware', action="store_true",
                        default=False, help='Computationally unaware simulation.')
    ...
    t = time.time()
    print(f"Loading Whisper {size} model for {language}...",
    ...
    asr = asr_cls(modelsize=size, lan=language,
    ...
    def output_transcript(o, now=None):
        ...
        if now is None:
            now = time.time()-start
        if o[0] is not None:
            # print("%1.4f %1.0f %1.0f %s" % (
            #     now*1000, o[0]*1000, o[1]*1000, o[2]), file=sys.stderr, flush=True)
            # print("%1.4f %1.0f %1.0f %s" %
            #       (now*1000, o[0]*1000, o[1]*1000, o[2]), flush=True)
    ...
    if args.offline:  # offline mode processing (for testing/debugging)
        ...
        while True:
            ...
            now = time.time() - start
            print(
                f"## last processed {end:.2f} s, now is {now:.2f}, the latency is {now-end:.2f}", file=sys.stderr, flush=True)
