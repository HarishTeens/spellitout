...
async def audio_stream(audio_file_path, encoding, sample_rate, channels, method):

    # url = "ws://localhost:5000"
    # To test integrating with DG, uncomment the following line (also, specify your API key below)
    url = 'wss://api.deepgram.com/v1/listen?model=general&tier=enhanced&language=hi'
        ...
        async def receiver(ws):
            ...
            async for msg in ws:
                ...
                if res.get("filename"):
                    ...
                    print(
                        f"🟢 (5/5) Sent audio data was stored in {raw_filename}")
...
def validate_encoding(encoding):
    dg_encodings = ["linear16", "flac", "mulaw",
                    "amr-nb", "amr-wb", "opus", "speex"]
    ...
    try:
        if input.lower().startswith("mic"):
            asyncio.get_event_loop().run_until_complete(
                audio_stream(input, encoding, sample_rate,
                             channels, method="mic")
            )
        else:
            asyncio.get_event_loop().run_until_complete(
                audio_stream(input, encoding, sample_rate,
                             channels, method=None)
            )
